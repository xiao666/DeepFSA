"""Trains the DeepMoji architecture on the SemEval2017 Task 5.
   last method load pretraining weights and just updates the last layer parameters.
"""
...
X_test,y_test=X_train[913:1142],y_train[913:1142]
X_train,y_train=X_train[0:913],y_train[0:913]
...
model = deepmoji_architecture(nb_classes=0, nb_tokens=NB_TOKENS, maxlen=maxlen,
                             embed_dropout_rate=0.2,final_dropout_rate=0.2,embed_l2=1E-6)
...
savebest=ModelCheckpoint(monitor='val_loss', filepath='micro_last_best',save_best_only=True)
...
print('Train...')
history =model.fit(X_train,y_train, batch_size=batch_size, epochs=64
