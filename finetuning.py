...
def calculate_batchsize_maxlen(texts):
    ...
    batch_size = 256 if maxlen <= 100 else 50 #changed 250 to  256
...
def chain_thaw(model, nb_classes, train, val, test, batch_size,
    ...
    # here model.evluate: _, acc = model.evaluate(X_test, y_test, batch_size=batch_size, verbose=0)


def train_by_chain_thaw(model, train_gen, val_data, loss,  epoch_size,callbacks,
    ...
    for layer in layers:
        ...
        for _layer in layers:
            if _layer is not None:
                ...
                change_trainable(_layer, trainable=trainable, verbose=False)#------------------------------------------------change trainable of layers
